...
void init(int width, int height, std::string strTitle, bool bFullScreen) {
	...
	modelfloresrojas.loadModel("../models/Plantas/10435_Begonia_v3_L3.obj");
	modelfloresrojas.setShader(&shaderMulLighting);
	...
	modelbuzz.loadModel("../models/Buzz2/torso.obj");
	modelbuzz.setShader(&shaderMulLighting);
	//NAVIDAD
...
bool processInput(bool continueApplication) {
	...
	if (glfwGetKey(window, GLFW_KEY_9) == GLFW_PRESS && sentido)
		...
		rot10 -= 0.01;
	//BUZ
	if (glfwGetKey(window, GLFW_KEY_F) == GLFW_PRESS && sentido)
		rotbuzz1 += 0.01;
	else if (glfwGetKey(window, GLFW_KEY_F) == GLFW_PRESS && !sentido)
		rotbuzz1 -= 0.01;
...
void applicationLoop() {
	...
	while (psi) {
		...
		model2 = glm::translate(model2, glm::vec3(0.9, -0.5, 11.5));
